...
class Node:
    ...
    def to_string_nodes(list):
        new_list = []
        for element in list:
            new_list.append(element.state)
        return new_list
...
def branch_and_bound(problem):
    """Search the deepest nodes in the search tree first. [p 74]"""
    return graph_search(problem, BABPriorityQueue())

def branch_and_bound_underestimation(problem):
    """Search the deepest nodes in the search tree first. [p 74]"""    
    return graph_search(problem, HeuristicPriorityQueue(problem))
...
class Resultado:
    def __init__(self, node, visitados, expandidos, cost):
        self.node = node
        self.visitados = visitados
        self.generados = expandidos
        self.cost = cost